/**
 * Переводит captains-en.ts на все поддерживаемые языки через Google Translate.
 * Использование: npx ts-node scripts/translateCaptainsAll.ts
 */
...
function readCaptainsFile(filePath: string): string[] {
  const content = fs.readFileSync(filePath, 'utf-8');
  const match = content.match(/export const captains = \[([\s\S]*?)\];/);
...
function writeCaptainsFile(filePath: string, names: string[]): void {
  const content = `export const captains = [\n${names
    .map(n => `  "${n.replace(/"/g, '\\"')}",`)
    .join('\n')}\n];\n`;
  fs.writeFileSync(filePath, content, 'utf-8');
}

async function translateCaptainsForLanguage(targetLang: string): Promise<void> {
  const projectRoot = process.cwd();
  const mocksDir = path.join(projectRoot, 'src/mocks');
  const sourcePath = path.join(mocksDir, 'captains-en.ts');
  const targetPath = path.join(mocksDir, `captains-${targetLang}.ts`);

  if (!languageMap[targetLang] || targetLang === 'en' || targetLang === 'ru') {
    throw new Error(`Пропуск языка: ${targetLang}`);
  }

  const names = readCaptainsFile(sourcePath);
  console.log(`Перевожу captains на ${targetLang} (${names.length} имён)...`);
  const translated = await translateNames(names, targetLang);
  writeCaptainsFile(targetPath, translated);
}
...
async function main() {
  ...
  console.log(
    `Перевод captains на ${targetLanguages.length} языков через Google Translate\n`,
  );

  for (const lang of targetLanguages) {
    try {
      await translateCaptainsForLanguage(lang);
      await delay(400);
    } catch (e) {
