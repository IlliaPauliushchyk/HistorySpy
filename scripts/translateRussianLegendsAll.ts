/**
 * Переводит russianLegends-en.ts на все поддерживаемые языки через Google Translate.
 * Использование: npx ts-node scripts/translateRussianLegendsAll.ts
 */
...
function readRussianLegendsFile(filePath: string): string[] {
  const content = fs.readFileSync(filePath, 'utf-8');
  const match = content.match(
    /export const russianLegends = \[([\s\S]*?)\];/,
  );
...
function writeRussianLegendsFile(
  filePath: string,
  names: string[],
): void {
  const content = `export const russianLegends = [\n${names
    .map(n => `  '${escapeForJsString(n)}',`)
    .join('\n')}\n];\n`;
  fs.writeFileSync(filePath, content, 'utf-8');
}

async function translateRussianLegendsForLanguage(
  targetLang: string,
): Promise<void> {
  const projectRoot = process.cwd();
  const mocksDir = path.join(projectRoot, 'src/mocks');
  const sourcePath = path.join(mocksDir, 'russianLegends-en.ts');
  const targetPath = path.join(mocksDir, `russianLegends-${targetLang}.ts`);

  if (!languageMap[targetLang] || targetLang === 'en' || targetLang === 'ru') {
    throw new Error(`Пропуск языка: ${targetLang}`);
  }

  const names = readRussianLegendsFile(sourcePath);
  console.log(
    `Перевожу russianLegends на ${targetLang} (${names.length} имён)...`,
  );
  const translated = await translateNames(names, targetLang);
  writeRussianLegendsFile(targetPath, translated);
}
...
async function main() {
  ...
  console.log(
    `Перевод russianLegends на ${targetLanguages.length} языков через Google Translate\n`,
  );

  for (const lang of targetLanguages) {
    try {
      await translateRussianLegendsForLanguage(lang);
      await delay(500);
    } catch (e) {
