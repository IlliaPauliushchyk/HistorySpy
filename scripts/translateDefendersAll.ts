/**
 * Переводит defenders-en.ts на все поддерживаемые языки через Google Translate.
 * Использование: npx ts-node scripts/translateDefendersAll.ts
 */
...
function readDefendersFile(filePath: string): string[] {
  const content = fs.readFileSync(filePath, 'utf-8');
  const match = content.match(/export const defenders = \[([\s\S]*?)\];/);
...
function writeDefendersFile(filePath: string, names: string[]): void {
  const content = `export const defenders = [\n${names
    .map(n => `  "${n.replace(/"/g, '\\"')}",`)
    .join('\n')}\n];\n`;
  fs.writeFileSync(filePath, content, 'utf-8');
}

async function translateDefendersForLanguage(targetLang: string): Promise<void> {
  const projectRoot = process.cwd();
  const mocksDir = path.join(projectRoot, 'src/mocks');
  const sourcePath = path.join(mocksDir, 'defenders-en.ts');
  const targetPath = path.join(mocksDir, `defenders-${targetLang}.ts`);

  if (!languageMap[targetLang] || targetLang === 'en' || targetLang === 'ru') {
    throw new Error(`Пропуск языка: ${targetLang}`);
  }

  const names = readDefendersFile(sourcePath);
  console.log(`Перевожу defenders на ${targetLang} (${names.length} имён)...`);
  const translated = await translateNames(names, targetLang);
  writeDefendersFile(targetPath, translated);
}
...
async function main() {
  ...
  console.log(
    `Перевод defenders на ${targetLanguages.length} языков через Google Translate\n`,
  );

  for (const lang of targetLanguages) {
    try {
      await translateDefendersForLanguage(lang);
      await delay(400);
    } catch (e) {
