/**
 * Переводит midfielders-en.ts на все поддерживаемые языки через Google Translate.
 * Использование: npx ts-node scripts/translateMidfieldersAll.ts
 */
...
function readMidfieldersFile(filePath: string): string[] {
  const content = fs.readFileSync(filePath, 'utf-8');
  const match = content.match(/export const midfielders = \[([\s\S]*?)\];/);
...
function writeMidfieldersFile(filePath: string, names: string[]): void {
  const content = `export const midfielders = [\n${names
    .map(n => `  "${n.replace(/"/g, '\\"')}",`)
    .join('\n')}\n];\n`;
  fs.writeFileSync(filePath, content, 'utf-8');
}

async function translateMidfieldersForLanguage(targetLang: string): Promise<void> {
  const projectRoot = process.cwd();
  const mocksDir = path.join(projectRoot, 'src/mocks');
  const sourcePath = path.join(mocksDir, 'midfielders-en.ts');
  const targetPath = path.join(mocksDir, `midfielders-${targetLang}.ts`);

  if (!languageMap[targetLang] || targetLang === 'en' || targetLang === 'ru') {
    throw new Error(`Пропуск языка: ${targetLang}`);
  }

  const names = readMidfieldersFile(sourcePath);
  console.log(`Перевожу midfielders на ${targetLang} (${names.length} имён)...`);
  const translated = await translateNames(names, targetLang);
  writeMidfieldersFile(targetPath, translated);
}
...
async function main() {
  ...
  console.log(
    `Перевод midfielders на ${targetLanguages.length} языков через Google Translate\n`,
  );

  for (const lang of targetLanguages) {
    try {
      await translateMidfieldersForLanguage(lang);
      await delay(400);
    } catch (e) {
