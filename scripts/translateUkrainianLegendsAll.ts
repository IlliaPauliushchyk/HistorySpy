/**
 * Переводит ukrainianLegends-en.ts на все поддерживаемые языки через Google Translate.
 * Использование: npx ts-node scripts/translateUkrainianLegendsAll.ts
 */
...
function readUkrainianLegendsFile(filePath: string): string[] {
  const content = fs.readFileSync(filePath, 'utf-8');
  const match = content.match(
    /export const ukrainianLegends = \[([\s\S]*?)\];/,
  );
...
function writeUkrainianLegendsFile(
  filePath: string,
  names: string[],
): void {
  const content = `export const ukrainianLegends = [\n${names
    .map(n => `  '${escapeForJsString(n)}',`)
    .join('\n')}\n];\n`;
  fs.writeFileSync(filePath, content, 'utf-8');
}

async function translateUkrainianLegendsForLanguage(
  targetLang: string,
): Promise<void> {
  const projectRoot = process.cwd();
  const mocksDir = path.join(projectRoot, 'src/mocks');
  const sourcePath = path.join(mocksDir, 'ukrainianLegends-en.ts');
  const targetPath = path.join(mocksDir, `ukrainianLegends-${targetLang}.ts`);

  if (!languageMap[targetLang] || targetLang === 'en' || targetLang === 'ru') {
    throw new Error(`Пропуск языка: ${targetLang}`);
  }

  const names = readUkrainianLegendsFile(sourcePath);
  console.log(
    `Перевожу ukrainianLegends на ${targetLang} (${names.length} имён)...`,
  );
  const translated = await translateNames(names, targetLang);
  writeUkrainianLegendsFile(targetPath, translated);
}
...
async function main() {
  ...
  console.log(
    `Перевод ukrainianLegends на ${targetLanguages.length} языков через Google Translate\n`,
  );

  for (const lang of targetLanguages) {
    try {
      await translateUkrainianLegendsForLanguage(lang);
      await delay(500);
    } catch (e) {
