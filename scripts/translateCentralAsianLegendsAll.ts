/**
 * Переводит centralAsianLegends-en.ts на все поддерживаемые языки через Google Translate.
 * Использование: npx ts-node scripts/translateCentralAsianLegendsAll.ts
 */
...
function readCentralAsianLegendsFile(filePath: string): string[] {
  const content = fs.readFileSync(filePath, 'utf-8');
  const match = content.match(
    /export const centralAsianLegends = \[([\s\S]*?)\];/,
  );
...
function writeCentralAsianLegendsFile(
  filePath: string,
  names: string[],
): void {
  const content = `export const centralAsianLegends = [\n${names
    .map(n => `  '${escapeForJsString(n)}',`)
    .join('\n')}\n];\n`;
  fs.writeFileSync(filePath, content, 'utf-8');
}

async function translateCentralAsianLegendsForLanguage(
  targetLang: string,
): Promise<void> {
  const projectRoot = process.cwd();
  const mocksDir = path.join(projectRoot, 'src/mocks');
  const sourcePath = path.join(mocksDir, 'centralAsianLegends-en.ts');
  const targetPath = path.join(mocksDir, `centralAsianLegends-${targetLang}.ts`);

  if (!languageMap[targetLang] || targetLang === 'en' || targetLang === 'ru') {
    throw new Error(`Пропуск языка: ${targetLang}`);
  }

  const names = readCentralAsianLegendsFile(sourcePath);
  console.log(
    `Перевожу centralAsianLegends на ${targetLang} (${names.length} имён)...`,
  );
  const translated = await translateNames(names, targetLang);
  writeCentralAsianLegendsFile(targetPath, translated);
}
...
async function main() {
  ...
  console.log(
    `Перевод centralAsianLegends на ${targetLanguages.length} языков через Google Translate\n`,
  );

  for (const lang of targetLanguages) {
    try {
      await translateCentralAsianLegendsForLanguage(lang);
