/**
 * Переводит youngTalents-en.ts на все поддерживаемые языки через Google Translate.
 * Использование: npx ts-node scripts/translateYoungTalentsAll.ts
 */
...
function readFile(filePath: string, exportName: string): string[] {
...
async function main() {
  ...
  const sourcePath = path.join(mocksDir, 'youngTalents-en.ts');
  const targetLanguages = Object.keys(languageMap).filter(
    l => l !== 'en' && l !== 'ru',
  );

  console.log(
    `Перевод youngTalents на ${targetLanguages.length} языков через Google Translate\n`,
  );

  const names = readFile(sourcePath, 'youngTalents');

  for (const lang of targetLanguages) {
    try {
      console.log(`Перевожу youngTalents на ${lang} (${names.length} имён)...`);
      const translated = await translateNames(names, lang);
      writeFile(
        path.join(mocksDir, `youngTalents-${lang}.ts`),
        'youngTalents',
        translated,
      );
      await delay(400);
